(function(){
  ...
  console.log('%c001.html ↔ 011.css ↔ 111.js | Comunicación Establecida', styleSub);
  ...
  galleryTabs.forEach(tab => {
    tab.addEventListener('click', () => {
      const targetGallery = tab.dataset.gallery; // 'destacados' o 'victorias'
      const panelId = (targetGallery === 'destacados') ? 'gallery-destacados' : 'gallery-victorias';
      
      // Si el panel ya está activo y hacemos clic en el mismo botón, lo ocultamos (toggle)
      ...
      if (currentActivePanel && currentActivePanel.id === panelId) {
        ...
        // Activar el nuevo panel y actualizar pestaña activa
  ...
  const WA_NUMBER = "5491123456789"; // Reemplazar con el número real, sin '+' y sin espacios
  ...
  // Validar ID (mínimo 10 caracteres, solo números y letras)
  ffIdInput.addEventListener('input', () => {
    const value = ffIdInput.value.trim();
    // Permitir números y letras (sin espacios)
  ...
  // Aplicar fade escalonado a elementos .view.active al cargar
  ...
  // Intersection Observer para aparición suave (ya existente, mejorado)
  ...
  // Efecto especial en juegos (sin cambios)
